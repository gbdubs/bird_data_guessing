...
func TestEggColor_AllAboutBirds(t *testing.T) {
	allAboutBirdsEggColor := func(englishName string) []*inference.String {
		return allAboutBirdsRequestForTesting(englishName).propertySearchers().eggColor.EggColor()
	}
	testSliceStringCases(t, allAboutBirdsEggColor,
		unorderedSliceStringCase("Black-and-white warbler", "Creamy white, pale bluish- or greenish-white, with speckles of brown or lavender"),
		unorderedSliceStringCase("Prothonotary warbler", "White spotted with rust-brown to lavender"),
		unorderedSliceStringCase("Anhinga", "Conspicuously pointed at one end, pale bluish green, and overlaid with a chalky coating"),
	)
}
